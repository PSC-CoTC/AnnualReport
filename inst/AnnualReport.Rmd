...
The ER Cap on the Interior Fraser MU is 20% due to its Low MU status, which has a survival component in addition to escapement.
...
t1 <- annual.tbl.second %>%
  ...
  mutate_if(colnames(.) %in% c("pre.cap", "pre.model", "post.cap", "post.estd"),  ~percent(.,accuracy = 0.1, suffix = "\\%")) %>%
  mutate(pre.cap = if_else(management.unit %in% c("Grays Harbor"), paste(pre.cap, footnote_marker_symbol(1, "latex",), sep = " "), pre.cap ))

table1form <- kable(t1, format = "latex", digits = c(0), format.args = list(big.mark =","), escape = F,  booktabs = T,
                    col.names = c("Managment unit", "Status", "Cap", "Model", "Status",  "Cap", "Estd", "Pre", "Post", "Pre", "Post"), label = "GUYUYUYUYUY", linesep = "") %>%
  kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  add_header_above(c(" ", "Pre-Season total ER" = 3, "Post-Season total ER" = 3, "Escapement" = 2, "Recruitment\n(Ocean Age-3)" = 2)) %>%
  footnote(symbol = c("The Grays Harbor ER caps in this report were computed using natural origin spawners, but ER caps for management purposes include all naturally spawning escapement (regardless of origin), and an escapement goal of 35,400 for a pre-season management cap of 28%.  "), threeparttable = T)
...
table3form <-  kable(t3, escape = F, booktabs= T, linesep = "", row.names = FALSE) %>%
  ...
  row_spec(c(12, 22, 24), background = "#e3e7fc") %>%
  row_spec(c(seq(1, nrow(t3),2)), background = "#d6d6d6") %>%
