...
big.bar.t1 <- filter(annual.tbl.second, management.unit == "Interior Fraser") %>%
  ...
  mutate(management.unit = "Interior Fraser without Big Bar")
...
# 
# big.bar.footnote <- str_glue("The Big Bar land slide resulted in an estimated {round(total.int.fr.esc - big.bar.esc, 0)} Coho spawner mortalities, resulting in a surviving escapement of {big.bar.esc} for IFC. If the Big Bar mortalities were not included this would have resulted in a post season ER of {round((total.int.fr.rec-big.bar.esc)/(total.int.fr.rec-big.bar.morts),3)}")
...
table3form <-  kable(t3, escape = F, booktabs= T, linesep = "", col.names = c(names(t3)[1:4], linebreak("Interior Fraser\nwithout bigbar"), names(t3)[6:14])) %>%
